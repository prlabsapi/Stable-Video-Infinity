...
class WanVideoSviProPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = WanVideoSviProPipeline(device=device, torch_dtype=torch_dtype)
        if use_usp:
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "prev_last_latent": prev_last_latent, "num_motion_latent": num_motion_latent,
        }
        ...
        return dict(video=video, prev_last_latent=prev_last_latent)
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image", "anchor"),
            output_params=("latents", "input_latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoSviProPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image, anchor=None):
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def encode_prompt(self, pipe: WanVideoSviProPipeline, prompt):
    ...
    def process(self, pipe: WanVideoSviProPipeline, prompt, positive) -> dict:
        pipe.load_models_to_device(self.onload_model_names)
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, input_image, end_image, height, width):
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_image", "end_image","anchor", "num_frames", "height", "width", "tiled", "tile_size", "tile_stride","inference_mode", "prev_last_latent", "auxiliary_video", "num_motion_latent"),
            output_params=("y",),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoSviProPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride, anchor=None, inference_mode=False, prev_last_latent=None, auxiliary_video=None, num_motion_latent=1):
        if input_image is None or not pipe.dit.require_vae_embedding:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        training_mode = not inference_mode

        # Normalize input_image to list
        input_image = [input_image] if not isinstance(input_image, list) else input_image
        input_image = [img.resize((width, height)) for img in input_image]
        anchor = anchor.resize((width, height))

        is_first_clip = (inference_mode and prev_last_latent is None)
        ...
        input_image = [pipe.preprocess_image(img).transpose(0, 1).to(pipe.device) for img in input_image]
        ...
        if is_first_clip:
            ...
            motion_latent = prev_last_latent.to(device=pipe.device)[:, -num_motion_latent:]
        ...
        y = torch.concat([msk, y]).unsqueeze(0).to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"y": y}
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride):
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, reference_image, height, width):
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, motion_bucket_id):
...
class WanVideoUnit_VAP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_UnifiedSequenceParallel(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline):
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_audio(self, pipe: WanVideoSviProPipeline, input_audio, audio_sample_rate, num_frames, fps=16, audio_embeds=None, return_all=False):
    ...
    def process_motion_latents(self, pipe: WanVideoSviProPipeline, height, width, tiled, tile_size, tile_stride, motion_video=None):
        pipe.load_models_to_device(["vae"])
        motion_frames = 73
        kwargs = {}
    ...
    def process_pose_cond(self, pipe: WanVideoSviProPipeline, s2v_pose_video, num_frames, height, width, tiled, tile_size, tile_stride, s2v_pose_latents=None, num_repeats=1, return_all=False):
    ...
    @staticmethod
    def pre_calculate_audio_pose(pipe: WanVideoSviProPipeline, input_audio=None, audio_sample_rate=16000, s2v_pose_video=None, num_frames=81, height=448, width=832, fps=16, tiled=True, tile_size=(30, 52), tile_stride=(15, 26)):
...
class WanVideoPostUnit_S2V(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, latents, motion_latents, drop_motion_frames):
...
class WanVideoUnit_AnimateVideoSplit(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, input_video, animate_pose_video, animate_face_video, animate_inpaint_video, animate_mask_video):
        if input_video is None:
            return {}
...
class WanVideoUnit_AnimatePoseLatents(PipelineUnit):
    ...
    def random_transform_pose_video(self, video_tensor, scale_range=(0.5, 1.5), translate_range=(-0.3, 0.3)):
        ...
        # Random translation (same for all frames, relative to image dimensions)
        ...
        # theta format: [[scale, 0, translate_x], [0, scale, translate_y]]
        theta = torch.tensor([
            [scale, 0, translate_x],
            [0, scale, translate_y]
        ], dtype=video_tensor.dtype, device=video_tensor.device).unsqueeze(0)  # [1, 2, 3]
        
        # Reshape video to [B*T, C, H, W] to apply same transformation to all frames at once
        video_reshaped = video_tensor.permute(0, 2, 1, 3, 4).contiguous()  # [B, T, C, H, W]
        video_reshaped = video_reshaped.view(B * T, C, H, W)  # [B*T, C, H, W]
        
        # Expand theta for all frames
        theta_expanded = theta.expand(B * T, -1, -1)  # [B*T, 2, 3]
        
        # Generate sampling grid (same transformation for all frames)
        grid = F.affine_grid(theta_expanded, video_reshaped.size(), align_corners=False)
        
        # Apply transformation with -1 as padding value (black background)
        transformed = F.grid_sample(video_reshaped, grid, mode='bilinear', 
                                   padding_mode='zeros', align_corners=False)
        # grid_sample's padding_mode='zeros' fills with 0, we need to set out-of-bound regions to -1
        # Create mask to identify out-of-bound regions
        mask_grid = F.grid_sample(torch.ones_like(video_reshaped), grid, mode='nearest', 
                                 padding_mode='zeros', align_corners=False)
        transformed = transformed * mask_grid + (-1.0) * (1 - mask_grid)
        
        # Reshape back to [B, C, T, H, W]
        transformed = transformed.view(B, T, C, H, W).permute(0, 2, 1, 3, 4).contiguous()
        
        return transformed

    def process(self, pipe: WanVideoSviProPipeline, animate_pose_video, tiled, tile_size, tile_stride, inference_mode=False):
        ...
        if not inference_mode:
            import random
            # import ipdb; ipdb.set_trace()
            # visualize_video_tensor(animate_pose_video, "before_pose.mp4", fps=16)
            # visualize_video_tensor(animate_pose_video, "after_pose.mp4", fps=16)
            if random.random() < 0.9:  # 80% probability to apply augmentation
                animate_pose_video = self.random_transform_pose_video(
                    animate_pose_video, 
                    scale_range=(0.5, 1.5),  # Scale range: 0.5x to 1.5x
                    translate_range=(-0.5, 0.5)  # Translation range: -50% to +50%
                )
...
class WanVideoUnit_AnimateInpaint(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, animate_inpaint_video, animate_mask_video, input_image, tiled, tile_size, tile_stride):
...
class WanVideoUnit_LongCatVideo(PipelineUnit):
    ...
    def process(self, pipe: WanVideoSviProPipeline, longcat_video):
